...
def search_ncbi(topic, max_results=10):
    """Search NCBI PubMed for a topic and return a list of PMIDs."""
...
def fetch_pubmed_details(pmids):
    """Fetch details for a list of PMIDs from PubMed in bulk."""
    if not pmids:
        return []
...
def send_discord_webhook_embed(article, webhook_url=None, username=None):
    """Send a PubMed article as an embed to a Discord webhook."""
...
for topic in config.get('topics', []):
    ...
    if elapsed < 0.5:
        # Sleep for the remaining time to ensure a minimum interval
        # otherwise ncbi will get mad
